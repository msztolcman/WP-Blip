...
class BlipApi {
	...
	/**
 	 ...
 	 * @var string
 	 */
	protected $_uagent		= 'BlipApi/0.1 (http://repo.urzenia.net/PHP:Blipapi)';
	...
	/**
 	 ...
 	 *
 	 * Uses methods {@link __query__get}, {@link __query__post}, {@link __query__put} and {@link __query__delete} to prepare data and connection params. Throws UnexpectedValueException exception for unknown method, or RuntimeException when CURL error exists.
 	 *
	...
	protected function _cmd__update_read ($id=null, $user=null, $include=array(), $since=false, $limit=10, $offset=0) {
		...
		# w odróżnieniu od samego API, chcemy ujednolicić pobieranie danych. podanie jako usera '__all__' powoduje pobranie update'ów od wszystkich userów. Układ RESTowych urli blipa jest co najmniej... dziwny... i mało konsekwentny.
		if ($user) {
	...
	/**
 	 ...
 	 * @return mixed return of {@link __query}
 	 */
	protected function _cmd__dirmsg_create ($body, $user) {
		if (!$body || !$user) {
			throw new UnexpectedValueException ('Directed_message body or recipient is missing.', -1);
		}
		return $this->__query ('/directed_messages', 'post', array ('directed_message[body]' => $body, 'directed_message[recipient]' => $user));
	}
	...
	/**
 	 ...
 	 * @return mixed return of {@link __query}
 	 */
	protected function _cmd__status_create ($body) {
		if (!$body) {
			throw new UnexpectedValueException ('Status body is missing.', -1);
		}
		return $this->__query ('/statuses', 'post', array ('status[body]' => $body));
	}
